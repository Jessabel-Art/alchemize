...
final class AlchemizeServiceRepository
{
  ...
  public function listAll(): array
  {
    $statement = $this->database->query(
      "SELECT * FROM services
             WHERE archived_at IS NULL AND catalog_status NOT IN ('NOT_OFFERED','FUTURE_EXPANSION')
             ORDER BY sort_order ASC, service_name ASC",
    );
    return array_map(
      fn(array $row): array => $this->withCatalogRelations($row),
      $statement->fetchAll(),
    );
  }
  ...
  public function listPublic(): array
  {
    ...
    return array_map(
      fn(array $row): array => $this->withPublicCatalogRelations($row),
      $statement->fetchAll(),
    );
  }
  ...
  {
    $statement = $this->database->prepare(
      "SELECT * FROM service_tiers WHERE id = :id LIMIT 1",
    );
    $statement->execute(["id" => $id]);
    $row = $statement->fetch();
  ...
  public function calculate(int $serviceId, ?int $tierId, array $inputs): array
  {
    ...
    if (
      $tierId !== null &&
      ($tier === null || (int) $tier["service_id"] !== $serviceId)
    ) {
      throw new AlchemizeRequestException(
        422,
        "VALIDATION_ERROR",
    ...
    return (new AlchemizeCatalogPricingService())->calculate(
      ...
      "tier_id" => $tierId,
      "catalog_version" =>
        (string) ($tier["catalog_version"] ?? $service["catalog_version"]),
    ];
  }
  ...
  private function withCatalogRelations(array $service): array
  {
    $tiers = $this->database->prepare(
      "SELECT * FROM service_tiers WHERE service_id = :service_id ORDER BY sort_order, tier_name",
    );
    $tiers->execute(["service_id" => $service["id"]]);
    $addons = $this->database->prepare(
      "SELECT * FROM service_addons WHERE service_id = :service_id AND archived_at IS NULL ORDER BY name",
    );
  ...
  private function withPublicCatalogRelations(array $service): array
  {
    ...
    $service["tiers"] = array_map(function (array $tier): array {
      ...
      if (is_array($limits)) {
        unset(
          $limits["implementation_hours"],
          $limits["max_internal_hours"],
          $limits["expected_labor_hours"],
        );
  ...
  public function assignToClient(
    ...
    $baseCatalogPrice =
      $tier["base_price"] ??
      ($tier["minimum_price"] ?? $service["default_price"]);
    ...
    if ($baseCatalogPrice === null || (float) $baseCatalogPrice <= 0) {
      throw new AlchemizeRequestException(
        422,
        "VALIDATION_ERROR",
        "This catalog item does not have an approved base price available.",
      );
    }

    $useCustomPrice = filter_var(
      $payload["use_custom_price"] ?? false,
      FILTER_VALIDATE_BOOLEAN,
    );
    $customOverride = null;
    if ($useCustomPrice) {
      $customOverride =
        isset($payload["custom_price_override"]) &&
        $payload["custom_price_override"] !== ""
          ? max(0, (float) $payload["custom_price_override"])
          : null;
      if ($customOverride === null || (float) $customOverride <= 0) {
        throw new AlchemizeRequestException(
          422,
          "VALIDATION_ERROR",
          "A valid custom or SOW price is required before assignment can be saved.",
        );
      }
    }
    ...
    $snapshot = [
      ...
      "billing_frequency" =>
        ...
        ($tier["billing_frequency"] ?? $service["billing_type"]),
      "catalog_version" =>
        $tier["catalog_version"] ?? $service["catalog_version"],
      "assigned_at" => date(DATE_ATOM),
      "custom_price_override" =>
        $customOverride !== null ? (float) $customOverride : null,
    ...
    $engagementTitle = trim(
      (string) ($payload["engagement_title"] ??
        $service["service_name"] .
          ($tier !== null ? " — " . $tier["tier_name"] : "")),
    );
    $description =
      trim((string) ($payload["description"] ?? ($notes ?? ""))) ?: null;

    $this->database->beginTransaction();
    try {
      $assignmentId = null;
      $assignmentStatement = $this->database->prepare(
        "INSERT INTO client_service_assignments (public_id,client_id,service_id,tier_id,agreed_base_price,agreed_recurring_amount,custom_price_override,selected_addons,billing_frequency,start_date,status,pricing_snapshot,catalog_version,notes) VALUES (:public_id,:client_id,:service_id,:tier_id,:agreed_base_price,:agreed_recurring_amount,:custom_price_override,:selected_addons,:billing_frequency,:start_date,:status,:pricing_snapshot,:catalog_version,:notes)",
      );
      ...
      $assignmentStatement->execute([
        ...
        "agreed_recurring_amount" =>
          isset($payload["agreed_recurring_amount"]) &&
          $payload["agreed_recurring_amount"] !== ""
            ? number_format(
              (float) $payload["agreed_recurring_amount"],
              2,
              ".",
              "",
            )
        ...
        "custom_price_override" =>
          $customOverride !== null
            ? number_format((float) $customOverride, 2, ".", "")
            : null,
        "selected_addons" => json_encode(
          (array) ($payload["selected_addons"] ?? []),
          JSON_THROW_ON_ERROR,
        ),
      ...
      $engagementNumber = trim(
        (string) ($payload["engagement_number"] ??
          "ENG-" . $clientId . "-" . time()),
      );
      $engagementStatus = in_array(
        (string) ($payload["engagement_status"] ?? "preparing"),
        [
          "preparing",
          "waiting_on_client",
      ...
      $itemStatement->execute([
        ...
        "catalog_default_price_snapshot" => number_format(
          (float) $baseCatalogPrice,
          2,
          ".",
          "",
        ),
  ...
  public function delete(int $id): void
  {
    ...
    if ($statement->rowCount() === 0) {
      throw new AlchemizeRequestException(
        404,
        "NOT_FOUND",
        "Service was not found or has already been deleted.",
      );
    }
  }
