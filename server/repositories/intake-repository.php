...
final class AlchemizeIntakeRepository
{
    ...
    public function referenceSnapshots(int $assignmentId,int $clientId):array {
        $s=$this->database->prepare('SELECT field_key, record_snapshot FROM intake_profile_references WHERE intake_assignment_id=:assignment AND client_id=:client');
        $s->execute(['assignment'=>$assignmentId,'client'=>$clientId]);$result=[];
        foreach($s->fetchAll() as $row)$result[$row['field_key']][]=array_intersect_key(json_decode($row['record_snapshot'],true,512,JSON_THROW_ON_ERROR),array_flip(['name','role_type','title','email','phone','ownership_percentage','label','address_type','line1','line2','city','state','postal_code','country']));
        return $result;
    }
