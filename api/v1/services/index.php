...
$bootstrapCandidates = array_filter([
  ...
  $documentRoot !== ""
    ? dirname($documentRoot) . "/alchemize-server/bootstrap.php"
    : null,
...
if ($bootstrap === null) {
  ...
  echo json_encode([
    "error" => [
      "code" => "INTERNAL_ERROR",
      "message" => "Services API is temporarily unavailable.",
    ],
  ]);
  exit();
}
...
try {
  ...
  if ($method === "POST" && $parts === []) {
    ...
    $id = $repository->create([
      ...
      "status" => in_array(
        ...
        : "active",
      "default_duration" =>
        isset($payload["default_duration"]) &&
        $payload["default_duration"] !== ""
          ? (int) $payload["default_duration"]
          : null,
      "billing_type" =>
        trim((string) ($payload["billing_type"] ?? "")) !== ""
          ? trim((string) ($payload["billing_type"] ?? ""))
          : null,
      "default_price" =>
        isset($payload["default_price"]) && $payload["default_price"] !== ""
          ? (string) $payload["default_price"]
          : null,
      "currency" =>
        trim((string) ($payload["currency"] ?? "USD")) !== ""
          ? strtoupper(trim((string) $payload["currency"]))
          : "USD",
      "active_flag" => isset($payload["active_flag"])
        ? (bool) $payload["active_flag"]
        : true,
      "billing_description" =>
        trim((string) ($payload["billing_description"] ?? "")) !== ""
          ? trim((string) ($payload["billing_description"] ?? ""))
          : null,
      "internal_pricing_notes" =>
        trim((string) ($payload["internal_pricing_notes"] ?? "")) !== ""
          ? trim((string) ($payload["internal_pricing_notes"] ?? ""))
          : null,
    ]);
  ...
  if (
    ...
    $payload = alchemize_read_json_request();
    $tierId =
      isset($payload["tier_id"]) && $payload["tier_id"] !== ""
        ? (int) $payload["tier_id"]
        : null;
    alchemize_json_response(
      [
        "data" => $repository->calculate(
          (int) $parts[0],
          $tierId,
          (array) ($payload["inputs"] ?? []),
        ),
      ],
      200,
    );
  ...
  if (
    ...
    alchemize_require_staff_or_admin();
    alchemize_require_csrf();
    $repository->delete((int) $parts[0]);
    alchemize_json_response(["data" => ["deleted" => true]], 200);
  }
  ...
  if (
    ...
    foreach (["default_duration", "default_price", "active_flag"] as $field) {
      if (array_key_exists($field, $payload)) {
        $values[$field] = $payload[$field] === "" ? null : $payload[$field];
      }
    }
    if (
      isset($payload["audience"]) &&
      in_array($payload["audience"], ["individual", "business", "all"], true)
    ) {
      $values["audience"] = $payload["audience"];
    }
    if (
  ...
  throw new AlchemizeRequestException(
    404,
    "NOT_FOUND",
    "The requested service route was not found.",
  );
  ...
  alchemize_error_response(
    500,
    "INTERNAL_ERROR",
    "Services API is temporarily unavailable.",
  );
}
